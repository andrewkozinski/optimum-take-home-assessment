...
### Running The Program:
To run the program, the TMDB_KEY environment variable must be set. You can set this either via the terminal or create a .env file in the backend directory containing it. 

Once the TMDB_KEY environment variable is set, the program should be able to run.

If using the terminal simply run
```
go run ./cmd/movieapi/main.go 
```
Or 
```
go build ./cmd/movieapi/main.go
```
to build an executable to run the program.
